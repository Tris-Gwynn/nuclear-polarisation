...
# Define spin quantum numbers (0.5 for 1H, 1.0 for 14N)
D_SPINS = [0.5] 
A_SPINS = [] 

# Hyperfine Tensors (in mT)
A_X = 0.005684 
A_Y = 0.005684 
A_Z = 0.005684 
...
# Electron-Electron Couplings (in mT)
J_EX = 0.0                      # Isotropic Exchange
D_TENSOR = np.zeros((3, 3))     # Dipolar tensor (explicit 3x3 zero matrix)

# --- B. External Magnetic Fields ---
# Static Field (B0)
B0 = 50*0.005684              # Static field strength (mT)
THETA = 0                    # Molecular orientation angle relative to B0
PHI = 0                      

# tNMR RF Driving Field (B1)
B1_RF_MT = 0*0.4                  # RF amplitude in mT (Set to 0.0 to disable RF field)
RF_FREQ_MHZ = 0*0.17               # 17 kHz converted to MHz
GYRO_1H_MHZ = 0*0.04258 * 2 * np.pi # Proton gyromagnetic ratio


# --- C. Initial State (Nuclear Polarization) ---
# Defined as vectors [Px, Py, Pz]
# E.g., Thermal = [0,0,0] | Z = [0,0,1] | X = [1,0,0]
Px = 0.0
Py = 0.0
Pz = 0.0
